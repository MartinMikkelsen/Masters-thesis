...
\begin{marginfigure}
	\centering
	\input{Figures/DressingOfProton}
...
For a given $E$, one can solve the second-order differential equation corresponding to $\phi(E)$. Conversely, for a given $\phi(r)$, one can compute the integral to find $E(\phi)$. This leads to the fixed-point equation given by
...
\subsection{Different Form Factors}\label{sec:formfactors}
...
The baryon-number-conserving chiral Lagrangian can be split into pieces with even numbers of fermion fields. In this section, we will focus on 
\begin{equation}
